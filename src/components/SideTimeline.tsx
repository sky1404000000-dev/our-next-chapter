...
const timelineItems = [
  { label: '인사말', href: '#invitation' },
  { label: '타임라인', href: '#schedule' },
  { label: '달력, 디데이', href: '#schedule' },
  { label: '오시는 길, 교통수단', href: '#location' },
  { label: '저희 커플 소개', href: '#about-us' },
  { label: '우리 이야기', href: '#our-story' },
  { label: '갤러리', href: '#gallery' },
  { label: '포항 가이드', href: '#pohang-guide' },
  { label: '마음전하실 곳', href: '#account' },
...
export default function SideTimeline({ showAccount }: SideTimelineProps) {
  ...
  const visibleItems = timelineItems.filter((item) => showAccount || item.href !== '#account');
  ...
  return (
    <>
      ...
      <aside className={`side-timeline ${isOpen ? 'is-open' : ''}`} aria-hidden={!isOpen}>
        ...
        <nav className="timeline-list" aria-label="타임라인 메뉴">
